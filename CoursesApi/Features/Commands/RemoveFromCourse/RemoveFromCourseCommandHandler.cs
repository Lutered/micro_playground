...
{
    public class RemoveFromCourseCommandHandler(
        IRequestClient<GetUserRequest> _client,
        ICourseRepository _courseRepo,
        IParticipantsRepository _participantsRepo
    ) : IRequestHandler<RemoveFromCourseCommand, HandlerResult>
    {
        public async Task<HandlerResult> Handle(RemoveFromCourseCommand request, CancellationToken cancellationToken)
        {
